...
void Map::drawGrid(QPainter& painter, int mapWidth, int mapHeight) {
    // Translate the coordinate system so that point 0 is in the center of the widget
    ...
    for (int x = -mapWidth / 2 + tickDistance; x < mapWidth / 2; x += tickDistance)
    {
        painter.drawLine(x, -5, x, 5);
        painter.drawText(x - 10, 20, QString::number(x));
    }
    for (int y = -mapHeight / 2 + tickDistance; y < mapHeight / 2; y += tickDistance)
    {
    ...
    for (int x = -mapWidth / 2 + tickDistance / 5; x < mapWidth / 2; x += tickDistance / 5)
    {
        painter.drawLine(x, -mapHeight / 2, x, mapHeight / 2);
    }
    for (int y = -mapHeight / 2 + tickDistance / 5; y < mapHeight / 2; y += tickDistance / 5)
    {
...
std::array<int, 3> Map::getDevicePose() {
    return curr_dev_pose->getPose();
}
...
//    qDebug() << text;
    ...
    prev_points.insert(prev_points.end(), std::make_move_iterator(curr_points.begin()),
